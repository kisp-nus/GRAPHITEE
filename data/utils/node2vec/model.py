...
class Node2vec(nn.Module):
    ...
    def __init__(self, g, embedding_dim, walk_length, p, q, num_walks=10, window_size=5, num_negatives=5,
        ...
        # try:
        #     self.embedding = torch.load('/home/gpzhang/Datasets/MIND/MINDsmall/node2vec.pth')
    ...
    def sample(self, batch):
        ...
        batch = batch.repeat(self.num_walks).type(torch.int32)
        # positive
        pos_traces = node2vec_random_walk(self.g, list(batch), float(self.p), float(self.q), int(self.walk_length), self.prob)
...
class Node2vecModel(object):
    ...
    def __init__(self, g, embedding_dim, walk_length, p=1.0, q=1.0, num_walks=1, window_size=5,
                 num_negatives=5, use_sparse=True, weight_name=None, device='cpu'):
    ...
    def _train_step(self, model, loader, optimizer, device):
        model.train()
        total_loss = 0
        for pos_traces, neg_traces in tqdm(loader):
    ...
    def train(self, epochs, batch_size, learning_rate=0.005):
        """
        Parameters
        ----------
        epochs: int
        ...
        """
        self.model.embedding = self.model.embedding.to(self.device)
        ...
        for i in range(epochs):
            ...
            if float(loss) < best_loss:
                ...
                emb = self.embedding().detach().cpu()
                torch.save(emb, '../retexo-datasets/MINDsmall/node2vec.pth')
                print('Node2Vec Updated !!')
