...
def encrypt_tensor(tensor, alias, kms):
    tensor_bytes = tensor.tobytes()
    encrypted_tensor = kms.encrypt(KeyId=f"alias/{alias}", Plaintext=tensor_bytes)
    binary_cipher_text = encrypted_tensor["CiphertextBlob"]
    base64_cipher_text = base64.b64encode(binary_cipher_text).decode()
    print(base64_cipher_text)
...
def main():
    ...
    if args.prepare is True:
        ...
        # Prepare tensors
        tensor1 = np.array([1], dtype=np.int32)
        tensor2 = np.array([2], dtype=np.int32)
        tensor3 = np.array([3], dtype=np.int32)

        tensors = [tensor1, tensor2, tensor3]
        encrypted_tensors = [encrypt_tensor(t, args.alias, kms) for t in tensors]

        with open("tensors.encrypted", "w") as file:
            json.dump(encrypted_tensors, file)
        exit()
        ...
        ciphertext = json.load(args.ciphertext)
        
        if not isinstance(ciphertext, list):
            print('Ciphertext should be a list of strings.')
            exit()
